...
export async function startServer(opts) {
    ...
    const app = new Hono();
    // --- Health ---
    app.get('/api/health', (c) => c.json({ status: 'ok', mode, ts: new Date().toISOString() }));
    ...
    if (existsSync(distPath)) {
        // Serve built assets
        app.use('/assets/*', serveStatic({ root: resolve(distPath, 'assets'), rewriteRequestPath: (p) => p.replace('/assets', '') }));

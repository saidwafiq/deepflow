 ...
 */
export async function parseCacheHistory(db, claudeDir) {
    ...
    for (let i = offset; i < lines.length; i++) {
        ...
        const ts = (record.timestamp ?? record.ts ?? new Date().toISOString());
        // Use existing session_id or synthesise one so FK is satisfied
        let sessionId = (record.session_id ?? record.sessionId);
        if (!sessionId) {
            sessionId = `cache-synthetic-${ts}`;
        }
        // Ensure session placeholder exists
        try {
            db.run(`INSERT OR IGNORE INTO sessions (id, user, tokens_in, tokens_out, cache_read, cache_creation, messages, tool_calls, cost, started_at)
         VALUES (?, 'unknown', 0, 0, 0, 0, 0, 0, 0, ?)`, [sessionId, ts]);
        }
        catch {
            // non-fatal
        }
        try {
            const rawCacheRead = (record.cache_read_tokens ?? record.cacheReadTokens ?? record.tokens ?? 0);
            const rawCacheCreation = (record.cache_creation_tokens ?? record.cacheCreationTokens ?? 0);
            const clampedCacheRead = Math.max(0, rawCacheRead);
            const clampedCacheCreation = Math.max(0, rawCacheCreation);
            if (rawCacheRead < 0)
                console.warn(`[ingest:cache-history] Clamping negative cache_read_tokens (${rawCacheRead}) to 0 at line ${i + 1}`);
            if (rawCacheCreation < 0)
                console.warn(`[ingest:cache-history] Clamping negative cache_creation_tokens (${rawCacheCreation}) to 0 at line ${i + 1}`);
    ...
    if (inserted > 0)
        console.log(`[ingest:cache-history] Inserted ${inserted} new records`);
}
//# sourceMappingURL=cache-history.js.map
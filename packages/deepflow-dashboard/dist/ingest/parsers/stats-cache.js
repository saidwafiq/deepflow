 ...
 */
export async function parseStatsCache(db, claudeDir) {
    ...
    if (seenRow && seenRow.value === contentSize)
        return; // no change
    let parsed;
    try {
    ...
    for (const s of sessions) {
        ...
        if (rawTokensIn < 0)
            console.warn(`[ingest:stats-cache] Clamping negative tokens_in (${rawTokensIn}) to 0 for session ${id}`);
        if (rawTokensOut < 0)
            console.warn(`[ingest:stats-cache] Clamping negative tokens_out (${rawTokensOut}) to 0 for session ${id}`);
        if (rawCacheRead < 0)
            console.warn(`[ingest:stats-cache] Clamping negative cache_read (${rawCacheRead}) to 0 for session ${id}`);
        if (rawCacheCreation < 0)
            console.warn(`[ingest:stats-cache] Clamping negative cache_creation (${rawCacheCreation}) to 0 for session ${id}`);
        if (rawCost < 0)
            console.warn(`[ingest:stats-cache] Clamping negative cost (${rawCost}) to 0 for session ${id}`);
        try {
    ...
    if (upserted > 0)
        console.log(`[ingest:stats-cache] Inserted ${upserted} session records from stats-cache`);
}

 ...
 */
export async function parseSessions(db, claudeDir) {
        ...
        for (const filePath of sessionFiles) {
            ...
            for (let i = offset; i < lines.length; i++) {
                ...
                if (resolvedModel && resolvedModel !== 'unknown')
                    model = resolvedModel.replace(/\[\d+[km]\]$/i, '');
                // Count messages by event type
                const eventType = event.type;
                if (eventType === 'assistant' || event.role === 'assistant')
                    messages++;
                if (eventType === 'user' || eventType === 'human' || event.role === 'human' || event.role === 'user')
                    messages++;
            ...
            if (tokensIn < 0) {
                console.warn(`[ingest:sessions] Clamping negative tokensIn (${tokensIn}) to 0 for session ${sessionId}`);
                tokensIn = 0;
            }
            if (tokensOut < 0) {
                console.warn(`[ingest:sessions] Clamping negative tokensOut (${tokensOut}) to 0 for session ${sessionId}`);
                tokensOut = 0;
            }
            if (cacheRead < 0) {
                console.warn(`[ingest:sessions] Clamping negative cacheRead (${cacheRead}) to 0 for session ${sessionId}`);
                cacheRead = 0;
            }
            if (cacheCreation < 0) {
                console.warn(`[ingest:sessions] Clamping negative cacheCreation (${cacheCreation}) to 0 for session ${sessionId}`);
                cacheCreation = 0;
            }
            ...
            if (rawCost < 0)
                console.warn(`[ingest:sessions] Clamping negative cost (${rawCost}) to 0 for session ${sessionId}`);
            ...
            if (hasNewData && sessionId) {
                ...
                if (existing) {
                    try {
                        db.run(`UPDATE sessions SET tokens_in = tokens_in + ?, tokens_out = tokens_out + ?,
               ...
               WHERE id = ?`, [tokensIn, tokensOut, cacheRead, cacheCreation, messages, toolCalls, cost,
                    ...
                    try {
                        db.run(`INSERT INTO sessions (id, user, project, model, tokens_in, tokens_out, cache_read, cache_creation, duration_ms, messages, tool_calls, cost, started_at, ended_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [sessionId, user, project, model, tokensIn, tokensOut, cacheRead, cacheCreation,

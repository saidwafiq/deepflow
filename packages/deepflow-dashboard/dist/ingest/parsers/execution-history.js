 ...
 */
function discoverExecutionHistoryFiles(claudeDir) {
    const results = [];
 ...
 */
export async function parseExecutionHistory(db, claudeDir) {
    ...
    for (const { path: filePath } of files) {
        ...
        for (const rec of newRecords) {
            ...
            let model = 'unknown';
            if (sessionId) {
                const tokenRows = db.all(`SELECT model, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens
           FROM token_events
           WHERE session_id = ? AND timestamp >= ? AND timestamp <= ?`, [sessionId, startedAt, endedAt]);
                for (const te of tokenRows) {
                    ...
                    if (te.model && te.model !== 'unknown')
                        model = te.model;
                }
            }
            const cost = computeCost(pricing, model, tokensIn, tokensOut, cacheRead, cacheCreation);
            try {
                db.run(`INSERT INTO task_attempts (task_id, spec, session_id, status, tokens_in, tokens_out, cache_read, cost, started_at, ended_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    end.task_id,
                    start.spec ?? null,
                    sessionId,

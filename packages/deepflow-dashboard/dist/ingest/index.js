 ...
 */
async function aggregateAndComputeCosts() {
  ...
  `);
    ...
    for (const s of sessions) {
        const modelPricing = resolveModelPricing(pricing, s.model);
        if (!modelPricing)
            continue;
        const inputCost = (s.tokens_in ?? 0) * (modelPricing.input ?? 0) / 1_000_000;
        const outputCost = (s.tokens_out ?? 0) * (modelPricing.output ?? 0) / 1_000_000;
        const cacheReadCost = (s.cache_read ?? 0) * (modelPricing.cache_read ?? modelPricing.input * 0.1) / 1_000_000;
        const cacheCreationCost = (s.cache_creation ?? 0) * (modelPricing.cache_creation ?? modelPricing.input * 1.25) / 1_000_000;
        const totalCost = inputCost + outputCost + cacheReadCost + cacheCreationCost;
        if (totalCost > 0) {
            run('UPDATE sessions SET cost = ? WHERE id = ?', [totalCost, s.id]);
            costUpdated++;
        }
    }
 ...
 */
function runMigrationSessionReparseV1() {
 ...
 */
function runMigrationToolQuotaReparseV1() {
 ...
 */
function runMigrationCostReparseV1() {
...
export async function runIngestion(deepflowDir) {

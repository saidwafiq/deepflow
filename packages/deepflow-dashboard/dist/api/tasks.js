...
tasksRouter.get('/', (c) => {
    ...
    const taskRows = all(`SELECT
     ...
     ORDER BY spec ASC, last_run_at DESC`, params);
...
tasksRouter.get('/:task_id/attempts', (c) => {
    ...
    const rows = all(`SELECT id, task_id, spec, session_id, status,
    ...
    const totalRow = all(`SELECT COUNT(*) AS total FROM task_attempts WHERE task_id = ?`, [taskId]);

...
sessionsRouter.get('/', (c) => {
    ...
    const rows = all(`SELECT * FROM sessions ${where} ORDER BY ${sort} ${order} LIMIT ? OFFSET ?`, [...params, limit, offset]);
    const totalRow = all(`SELECT COUNT(*) as total FROM sessions ${where}`, params);

...
- **Working directory contract** (CRITICAL): the prompt's first line declares `WORKDIR: <path>`. Run `cd <WORKDIR>` ONCE as your first Bash call; your shell session keeps the cwd across subsequent invocations, so you do NOT need to re-prepend it. All Read/Write paths MUST be absolute and rooted at `<WORKDIR>`. All git operations MUST still use `git -C <WORKDIR>` form (belt-and-suspenders). NEVER run `git commit`, `git add`, or `git checkout` without `-C` — the curator's cwd is the main repo, and untargeted git ops will land on `main`.
